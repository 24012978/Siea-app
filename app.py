...
app = Flask(__name__)
app.secret_key = "siea_app_produccion_playstore_2026"

# Base de datos general de usuarios registrados
...
def registro():
 ...
 if request.method == "POST":
  ...
  if not nombre or not correo or not telefono or not u or not p:
   msg = "Todos los campos son obligatorios para el registro oficial."
  else:
   us = ld_u()
   if u in us:
 ...
 h+='.card-login{width:92%;max-width:400px;background:#0A2F0A;border:2px solid #FFD700;border-radius:16px;padding:22px;box-shadow:0 10px 30px rgba(0,0,0,0.8);box-sizing:border-box}'
...
# --- 2. PANTALLA DE LOGIN INSTITUCIONAL ORIGINAL CON PAGOS Y WHATSAPP ---
...
def login_page():
 ...
 h+='.card-login{width:94%;max-width:420px;background:#0A2F0A;border:2px solid #4CAF50;border-radius:16px;padding:24px;box-shadow:0 10px 30px rgba(0,0,0,0.8);box-sizing:border-box}'
 ...
 h+='<div class=info-box><b>Propósito Institucional:</b> Plataforma avanzada para la gestión, práctica y evaluación de reactivos oficiales. Diseñada para garantizar el máximo rendimiento académico y profesional.<br><br><b>Aviso de Privacidad:</b> Sus datos e historial se encuentran cifrados y protegidos de forma independiente por usuario conforme a la ley aplicable.</div>'
 if msg: h+=f'<div class=msg>{msg}</div>'
 h+='<form method=post>'
 ...
 h+='<a href="https://wa.me/528110290152?text=Hola,%20necesito%20soporte%20o%20información%20sobre%20la%20app%20SIEA" target=_blank class=btn-pay>💳 ACTIVAR SUSCRIPCIÓN / PAGOS</a>'
 h+='<div class=whatsapp>📞 Soporte WhatsApp: <a href="https://wa.me/528110290152" target=_blank>+52 81 1029 0152</a></div>'
 h+='</div>'
 return h
...
# --- 3. ESTRUCTURA BASE Y DISEÑO VÍVIDO ---
def base(t,b):
 u=session.get("user","")
...
def panel_principal():
 ...
 if not ma:
  ...
  for idx,m in enumerate(materias):
   c=ms.get(m,0)
   _,col=colores[idx % len(colores)]
   h+=f'<div class=card card-hi><div style=display:flex;justify-content:space-between;align-items:center;margin-bottom:8px><span style=font-weight:900;font-size:14px;color:#FFD700>📘 {m}</span><span class=badge style=background:{col};color:#fff>{c} PREGUNTAS</span></div>'
  ...
  if total_preg>0:
   ...
   h+=f'<div class=card card-gen-vacio><b style=color:#aaa>⭐ EXAMEN GENERAL (SIN REACTIVOS)</b><p style=font-size:11px;color:#888;margin:6px 0>Agregue preguntas en las materias para habilitar</p><button class="btn" style=background:#333;color:#777 disabled>🚀 MODULO INHABILITADO</button></div>'
  
  h+='<div class=card card-per><b style=color:#BA68C8>🎯 EXAMEN PERSONALIZADO</b><p style=font-size:11px;color:#E1BEE7;margin:6px 0>Elija qué materias desea combinar en su evaluación</p><form method=post action=/exam_personalizado>'
  for m in materias:
   c=ms.get(m,0)
 ...
 allp=ld()
 b='<div style=margin:12px;display:flex;gap:8px><a href=/cambiar class=btn-inicio>🏠 VOLVER AL PANEL</a></div>'
 ...
 if len(pr)==0:
  b+=f'<div class=card style=background:#2A1818;border:1px solid #D32F2F><b style=color:#FF8A80>⚠️ Aún no hay reactivos registrados en {ma}</b></div>'
 for p in allp:
...
# --- 5. HISTORIAL CON MEJOR PUNTAJE ESCALONADO ---
@app.route("/historial")
def hi():
 h=ld_h()
 ...
 if not h:
  b+='<div class=card>No hay exámenes registrados en el historial todavía.</div>'
 else:
  # Encontramos la calificación máxima para destacar el mejor examen escalonado
  max_por = max([x["por"] for x in h]) if h else 0
  for idx, x in enumerate(reversed(h)):
   es_mejor = (x["por"] == max_por and idx == 0) # Destaca el más reciente con la mayor calificación
   borde_estilo = 'border:2px solid #FFD700; background:linear-gradient(180deg,#2E2605,#1A1503);' if es_mejor else 'background:linear-gradient(180deg,#0F380F,#082008); border-left:6px solid #4CAF50;'
   etiqueta_mejor = '<span class=badge style=background:#FFD700;color:#000;margin-bottom:6px>🏆 MEJOR PUNTAJE</span><br>' if es_mejor else ''
   
   b+=f'<div class=card style="{borde_estilo}">{etiqueta_mejor}<b style=color:#FFD700;font-size:13px>{x["titulo"]}</b><br><span style=color:#81C784>Calificación: <b>{x["por"]}%</b></span><br><small style=color:#aaa>{x["fecha"]}</small></div>'
