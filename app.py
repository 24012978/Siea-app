...
def login_page():
 ...
 h+='<div id=register-form class="tab-content">'
  ...
  h+='<div class=lbl>CREAR USUARIO</div><input class=inp name=new_user>'
  h+='<div class=lbl>NOMBRE COMPLETO</div><input class=inp name=new_name>'
  h+='<div class=lbl>CORREO ELECTRONICO</div><input class=inp name=new_email>'
  h+='<div class=lbl>CREAR CONTRASEÑA</div><input class=inp type=password name=new_pass>'
  h+='<div class=lbl>CONFIRMAR CONTRASEÑA</div><input class=inp type=password name=confirm_pass>'
  h+='<button class=btn>✅ CREAR CUENTA</button>'
  h+='</form>'
  h+='</div>'
  h+='<div class=legal>'
  h+='<b>AVISO DE CONFIDENCIALIDAD Y MARCO LEGAL:</b> La informacion contenida, procesada y almacenada en esta plataforma es de uso institucional exclusivo.'
  h+='</div></div>'
  h+='<div class=card-support>'
  h+='<div style="font-weight:900;letter-spacing:2.5px;color:#FFD700;font-size:12px">S.I.E.A. SOPORTE</div>'
  ...
  h+='<p style="font-size:10px;color:#FFD700;margin-top:10px">Disponible de Lunes a Viernes 9am - 6pm</p>'
  h+='</div>'
  h+='<div style="border-left:3px solid #a855f7;background:#1a102a;padding:15px;border-radius:8px;margin-top:15px">'
  h+='<b style="color:#a855f7;">SUSCRIPCION</b>'
  h+='<p>Acceso ilimitado a todas las materias.</p>'
  h+='<a href="https://wa.me/528110290152" target="_blank" style="display:block;background:#a855f7;color:#fff;padding:10px;border-radius:8px;text-align:center;text-decoration:none;margin-top:10px;font-weight:900">CONTRATAR POR WHATSAPP</a>'
  h+='</div>'
...
def base(t,b):
  u=session.get("user","")
  frases=["( La disciplina es la clave del éxito operativo )", "( La constancia vence lo que la suerte no puede )"]
  f_activa = random.choice(frases)
  h=""
  h+='<meta name=viewport content="width=device-width,initial-scale=1">'
  h+='<link href="https://fonts.googleapis.com/css2?family=Montserrat:ital,wght@0,400;0,700;0,900;1,400&display=swap" rel=stylesheet>'
  h+='<style>'
  h+='body{margin:0;background:#050B05;color:#E0EFE0;font-family:Montserrat,system-ui}'
  h+='.top{background:linear-gradient(90deg,#00FF80,#00B359);color:#050905;padding:10px 15px}'
  h+='.bar{background:#0D1A0D;padding:10px 12px;display:flex;justify-content:space-between}' h+='.btn-inicio{background:linear-gradient(180deg,#00FF80,#00B359);color:#050905;padding:10px 18px;border-radius:8px;text-decoration:none;font-size:12px;font-weight:900;letter-spacing:1px;display:inline-block}'
